...
### Description
E-Comerce Core is a sample microservice code base build for 1 project apply e-comerce-logic. This project includes most common pattern in MSA, especially:
- Saga
- CQRS (query service)
...
Create network
```commandline
docker network create -d bridge anhtuan-network
```
Run infra
```commandline
...
Enable consul acl
```commandline
docker exec -it anhtuan-infra-consul consul acl bootstrap
```

Remember the SecretID printed on the command line. It is your consul token.

```commandline
docker exec -it anhtuan-infra-consul sh
```
```commandline
...
then click **Enable new engine**. Choose **KV**, set path is **anhtuan** and click **Enable Engine**.
...
```json
{
  "CONSUL_HOST": "anhtuan-ìnfra-consul",
  "CONSUL_PORT": "8500",
  "CONSUL_TOKEN": "200274eb-bdf5-3324-d065-28ecd61ff8be",
  "DB_HOST": "52.79.219.185",
  "DB_PASSWORD": "123456",
  ...
  "RABBITMQ_PORT": "5672",
  "RABBITMQ_USERNAME": "anhtuan",
  "ZIPKIN_HOST": "anhtuan-ìnfra-zipkin",
  "ZIPKIN_PORT": "9411"
}
```
